...
# from dataset_one import DAVIS_MO_Test
...
# Model and version
MODEL = 'STM'
print(MODEL, ': run for Gui segmentation)
...
def Run_video(Fs, Ms, num_frames, num_objects, Mem_every=None, Mem_number=None):
    ...
    check = 40 ##--- number of models to augment 

    Es = torch.zeros_like(Ms)
    Es[:,:,0] = Ms[:,:,0]

    EsMC = torch.zeros_like(Ms)
    Es_V = torch.zeros_like(Ms)
    Unc=torch.zeros_like(Ms)
    map_Es_V = []

    Norm_Es_V = torch.zeros_like(Ms)
    Norm_Es_V[:,:,0] = Ms[:,:,0]

    E_probs = torch.zeros(Ms.size()[0], Ms.size()[1], Ms.size()[2], Ms.size()[3], Ms.size()[4], check) 

    for t in tqdm.tqdm(range(1, num_frames)): # num_frames
        ...
        for snum in range(1, check):    
            with torch.no_grad():
                logit = model(Fs[:,:,t], this_keys, this_values, torch.tensor([num_objects]))
            Es[:,:,t] = F.softmax(logit, dim=1)
            E_probs[:,:,t,:,:,snum] = F.softmax(logit, dim=1)
        
        EsMC[:,:,t] = torch.mean(E_probs[:,:,t], 4)
        
        Es_V[:,:,t] = torch.var(E_probs[:,:,t], 4)
                       
        if t-1 in to_memorize:
            keys, values = this_keys, this_values
       
        min_v = torch.min(Es_V[:,:,t])
        max_v = torch.max(Es_V[:,:,t])
        Norm_Es_V[:,:,t] = 255*(Es_V[:,:,t] - min_v)/(max_v - min_v)
        Norm_Es_V_arr = Norm_Es_V[0].numpy().astype(np.uint8)

    pred = np.argmax(EsMC[0].cpu().numpy(), axis=0).astype(np.uint8)
    return pred, EsMC, Es_V, Norm_Es_V_arr
...
pth_path = '/home/ubuntu/Downloads/Uncertainty_STM/STM_weights.pth' ##---
...
#seq: 0 , V : tensor1, tensor2, tensor like int (2) , info (name,num_frames,size_480p)
for seq, V in enumerate(Testloader):
    ...
    pred, EsMC, Es_V, Norm_Es_V_arr = Run_video(Fs, Ms, num_frames, num_objects, Mem_every=5, Mem_number=None)
        
    # Save results for quantitative eval ######################
    #change to fit your path
    test_path = os.path.join('/home/ubuntu/Downloads/Uncertainty_STM/test', code_name, seq_name)
    ...
    for f in range(num_frames):
        ...
        # # # save uncertainty Map: needs normalization
        k = Norm_Es_V_arr[0,f]
        plt.imshow(k, cmap='seismic')
        #plt.colorbar() 
        plt.axis('off')
        plt.savefig(os.path.join(test_path, '{:05d}_Umap.png'.format(f)), bbox_inches='tight', pad_inches = 0)
        plt.close('all')
    Unc=Es_V[0].numpy()
    Unc=np.mean(Unc,3)
    Unc=np.mean(Unc,2)
    Unc=np.mean(Unc,0)
    #save Uncertainty Results each frame. 
    np.savetxt("UncertaintySave.txt", Unc, fmt='%.8e', delimiter=',')
...
m = running_time // 60
s = running_time - m*60
print("Running_Time: {}h {}m {}s".format(h, m, s))
